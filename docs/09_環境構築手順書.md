...
このプロジェクトでは、Backend起動時に `schema.sql` と `data.sql` は自動実行しません。
初回構築時、またはDBを初期化し直したい場合は、Backend起動前にSQLを手動実行してください。
...
`schema.sql` は `management_app` スキーマとテーブルを作成します。
`data.sql` は初期データを投入します。

リポジトリのルートディレクトリで以下を実行します。

```cmd
psql -h localhost -p 5432 -U postgres -d postgres -f backend/src/main/resources/sql/schema.sql
psql -h localhost -p 5432 -U postgres -d postgres -f backend/src/main/resources/sql/data.sql
```
...
1. PostgreSQL を起動する
2. 初回構築時は `schema.sql` と `data.sql` を手動実行する
...
- `schema.sql` / `data.sql` を手動実行済みであること
...
- `backend/src/main/resources/sql/data.sql` の初期データが投入されていること
